...
export const scikitLearnCourse: CourseModule = {
  ...
  quiz: [
    { id: 'sk-q1', prompt: 'What is the purpose of train_test_split?', options: ["To make the dataset smaller", "To speed up training", "To evaluate model generalization on unseen data", "To shuffle the columns"], answerIndex: 2, explanation: 'We split data to ensure the model learns patterns rather than just memorizing the training set.' },
    { id: 'sk-q2', prompt: 'Which metric measures: "Out of all actual positives, how many were found?"', options: ["Precision", "Accuracy", "R-squared", "Recall"], answerIndex: 3, explanation: 'Recall (or sensitivity) is True Positives / (True Positives + False Negatives).' },
    ...
    { id: 'sk-q4', prompt: 'What does cross-validation help prevent?', options: ["Syntax errors", "Overfitting to a specific train/test split", "Missing values", "Data leakage"], answerIndex: 1, explanation: 'By evaluating across multiple splits, CV provides a more robust performance estimate.' },
    { id: 'sk-q5', prompt: 'Which algorithm is used for Classification?', options: ["Linear Regression", "Mean Squared Error", "Logistic Regression", "PCA"], answerIndex: 2, explanation: 'Despite its name, Logistic Regression is used for discrete classification tasks.' },
    { id: 'sk-q6', prompt: 'What does a Confusion Matrix show?', options: ["The learning rate curve", "Feature importances", "The dataset schema", "True Positives, False Positives, True Negatives, False Negatives"], answerIndex: 3, explanation: 'It is a table showing correct and incorrect predictions broken down by class.' },
    ...
    { id: 'sk-q8', prompt: 'What is a hyperparameter?', options: ["A learned weight matrix", "A configuration set before training (e.g., max depth)", "The target variable y", "The learning curve"], answerIndex: 1, explanation: 'Hyperparameters govern the training process itself and are not learned from data.' },
    { id: 'sk-q9', prompt: 'If your dataset is 99% normal and 1% fraud, why is Accuracy a bad metric?', options: ["Accuracy cannot be computed", "It requires floating point numbers", "A model predicting \"always normal\" gets 99% accuracy but catches no fraud", "It takes too long to calculate"], answerIndex: 2, explanation: 'Accuracy hides poor performance on minority classes. F1 score or Recall is better here.' },
    { id: 'sk-q10', prompt: 'What is the correct scikit-learn method order?', options: ["model.predict(X) -> model.fit(y)", "model.train() -> model.test()", "model.load(X) -> model.run()", "model.fit(X, y) -> model.predict(X_new)"], answerIndex: 3, explanation: 'The standard API requires fitting the model to data first, then predicting on new data.' },
  ],
  practice: [
