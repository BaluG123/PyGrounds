...
export const deepLearningCourse: CourseModule = {
  ...
  quiz: [
    { id: 'dl-q1', prompt: 'What does a single artificial neuron do?', options: ["Stores data in a SQL database", "Sorts an array", "Generates random text", "Computes a weighted sum of inputs plus bias, then applies an activation"], answerIndex: 3, explanation: 'A neuron performs a dot product and passes it through an activation function.' },
    ...
    { id: 'dl-q3', prompt: 'What does the ReLU function do?', options: ["Smashes all values between 0 and 1", "Returns x if x > 0, else 0", "Returns the square root of x", "Adds 1 to x"], answerIndex: 1, explanation: 'ReLU (Rectified Linear Unit) is simply max(0, x).' },
    { id: 'dl-q4', prompt: 'In a forward pass, how do we compute a whole layer at once?', options: ["A for-loop over every neuron", "Random sampling", "Matrix multiplication", "Sorting"], answerIndex: 2, explanation: 'Matrix multiplication (dot product) computes all neuron outputs simultaneously, which GPUs excel at.' },
    { id: 'dl-q5', prompt: 'What is a Loss Function?', options: ["The function that turns inputs to outputs", "A way to compress data", "The learning rate schedule", "A measure of how wrong the network's predictions are"], answerIndex: 3, explanation: 'Loss (or Cost) tells the network the difference between its prediction and the true label.' },
    ...
    { id: 'dl-q7', prompt: 'What does the Learning Rate control?', options: ["How fast the code compiles", "The step size taken during weight updates", "The number of layers in the network", "The batch size"], answerIndex: 1, explanation: 'It multiplies the gradient to determine how aggressively to change the weights.' },
    { id: 'dl-q8', prompt: 'What happens if the learning rate is too high?', options: ["The model will train too perfectly", "It will require more memory", "The model might overshoot the minimum and fail to converge", "The network becomes a decision tree"], answerIndex: 2, explanation: 'A massive learning rate causes the weights to bounce around wildly (divergence).' },
    { id: 'dl-q9', prompt: 'What is an Epoch?', options: ["One weight update", "A single forward pass", "The initialization phase", "One complete pass through the entire training dataset"], answerIndex: 3, explanation: 'Training usually requires multiple epochs (passes over the full dataset).' },
